...
{
    [Serializable]
    public class RawImageProMaskExtend
    {
        [SerializeField]
        public bool IsUseMask = false;
        /// <summary>
        /// 内切圆半径 图片的一半差不多就是一个圆了 这里相当于图片十分之一的长度
        /// </summary>
        ...
        public void Initializa(RawImage rawImage)
        {
            m_RectTransform = rawImage.rectTransform;
            m_Color = rawImage.color;
        }
        public void OnPopulateMesh(VertexHelper vh)
        {
            vh.Clear();
            float tw = m_RectTransform.rect.width * Scale; //图片的宽
            float th = m_RectTransform.rect.height * Scale; //图片的高
            float twr = tw / Scale / 2;
            float thr = th / Scale / 2;
            ...
            for (int j = 0; j < CirclePoint.Count; j += 2)
            {
                ...
                for (int i = 0; i < TriangleNum; i++)
                {
                    ...
                    pos2 = new Vector2(CirclePoint[j].x + radius * Mathf.Cos(curDegree),
                        CirclePoint[j].y + radius * Mathf.Sin(curDegree));
