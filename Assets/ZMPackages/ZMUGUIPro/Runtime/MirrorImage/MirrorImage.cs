...
public class MirrorImage : Image
{
    ...
    void GenerateSimpleSprite(VertexHelper vh, bool lPreserveAspect)
    {
        ...
        Vector4 v1 = v;
        //Debuger.Log("uv::::" + uv + "  v:" + v+ "  center:"+ rectTransform.rect.center+ "  rect:" + rectTransform.rect);
    ...
    {
        //Debuger.Log($"posMin:{posMin},posMax:{posMax},uvMin:{uvMin},uvMax:{uvMax}");
    ...
    void GenerateTiledSprite(VertexHelper toFill)
    {
        ...
        if (sprite != null && (hasBorder || sprite.packed || sprite.texture.wrapMode != TextureWrapMode.Repeat))
        {
            ...
            if (fillCenter)
            {
                ...
                {
                    //Debuger.Error("Too many sprite tiles on Image \"" + name + "\". The tile size will be increased. To remove the limit on the number of tiles, convert the Sprite to an Advanced texture, remove the borders, clear the Packing tag and set the Wrap mode to Repeat.", this);
